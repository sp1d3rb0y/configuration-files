...
class RopeMode(object):
    ...
    def open_project(self, root=None):
        if not root:
            # FIX SPIDERBOY
            if self.env.get('autocreate_project'):
                root = self.env.ask_directory('Rope PONEY root folder: ')
            else:
        ...
        if not os.path.exists(address):
            # FIX SPIDERBOY
            if not self.env.get('autocreate_project'):
                if not self.env.y_or_n('Project not exists in %s, ' \
                                       'create one?' % root):
                    self.env.message("Project creation aborted")
                    return
            else:
                self.env.message('Project Autocreated successfully')
